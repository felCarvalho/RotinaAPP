...
export async function deleteCategory({
  ...
  const validate = schemaDeleteCategory.safeParse(form);
  ...
  try {
    const response = await axios.delete(
      `/home/rascunhos/deletar-categoria/${validate.data.idCategory}`,
      {
        baseURL: LOCAL_URL,
        headers: {
    ...
    return data(
      {
        message: "Ops! erro interno ao deletar a categoria",
        error: error,
      },
      {
        status: 500,
