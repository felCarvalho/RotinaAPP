...
const schemaUpdateCategoryTask = z.object({
  idCategory: z.string().min(1, { message: "ID da categoria inválido" }),
  completed: z.string().min(1, { message: "Status de conclusão inválido" }),
  statusCategory: z.string().min(1, { message: "Status da categoria inválido" }),
});
...
export async function updateCategoryTask({
  ...
  const validate = schemaUpdateCategoryTask.safeParse(form);
  ...
  try {
    const response = await axios.put(
      "/home/categorias/atualizar-rotina",
      {
        idCategory: validate.data.idCategory,
        completed: validate.data.completed,
        statusCategory: validate.data.statusCategory,
      },
      {
        baseURL: LOCAL_URL,
        headers: {
    ...
    return data(
      {
        message: "Ops! erro interno ao atualizar as tarefas da categoria",
        error: error,
      },
      {
        status: 500,
