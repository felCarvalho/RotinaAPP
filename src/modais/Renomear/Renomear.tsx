...
export function Renomear() {
  ...
  const [valueRenomear, setValueRenomear] = useState<string>("");
  ...
  function Handle(e: React.ChangeEvent<HTMLInputElement>) {
    setValueRenomear(e.target.value);
  }
  ...
  useEffect(() => {
    if (buscarTask) {
      setValueRenomear(buscarTask?.rotina);
    }
  }, [buscarTask]);
  ...
  function onSubmit(e: React.ChangeEvent<HTMLFormElement>) {
    ...
    const validar = verificarRenoemar({ value: valueRenomear, id: renomear });

    if (!validar) {
      return;
    }

    renomearTask({ id: validar?.id, task: validar?.value });
    setTimeout(() => {
      setQuery(null);
    }, 300);
  }
          ...
          <div className="absolute right-2 left-2 rounded-[50px] bg-white shadow-2xl shadow-blue-100">
            ...
            <form className="mt-20 px-5" onSubmit={onSubmit}>
              <label className="flex flex-col gap-1">
                ...
                <Input type="text" name="renomear" placeholder="renomear rotina..." value={valueRenomear} onChange={Handle} />
              </label>
