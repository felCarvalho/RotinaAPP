      ...
      {verificarWidth({ largura: 1000 }) && (
        <div className={`flex h-full w-full items-center !justify-start gap-5 p-5 ${layout()}`}>
          ...
          <div className="min-h-max">
            <Button
              type="button"
              onClick={() => setLayout({ isDesktopLayout: !isLayout?.isDesktopLayout, isMobileLayout: isLayout?.isMobileLayout })}
            >
              <i>
          ...
          <div className="h-full w-full">{<Outlet context={{ setLayout, isLayout }} />}</div>
        </div>
      )}
      {!verificarWidth({ largura: 1000 }) && (
        <div className="flex h-full w-full items-center !justify-start gap-10 p-5">
          <motion.div
            initial={{ y: 0 }}
            animate={isLayout?.isMobileLayout ? { y: -1100 } : { y: 0 }}
