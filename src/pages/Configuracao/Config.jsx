...
export function Config() {
  ...
  return (
    <AnimatePresence>
      <Overlay>
        <div className="absolute right-2 left-2 rounded-[50px] bg-white p-5 shadow-sm shadow-blue-50">
          <HeaderContent
            btnBack={() => navigate("/inicio")}
          ...
          <div className="box-border overflow-auto pt-8">
            {DataConfig.map((c) => (
              <div
                key={c?.id}
                className="mb-3 box-border flex flex-row items-center justify-between rounded-full bg-blue-100/5 p-5 shadow-sm shadow-blue-50"
                onDoubleClick={() => {
                  verificarConfigStatus({ chave: c?.id }) ? setStatusFunction({ [c?.id]: !statusFunction?.[c?.id] }) : null;
                }}
              >
                <NavLink>
